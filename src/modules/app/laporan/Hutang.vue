...
export default {
    data () {
        return {
            ...
            result:{
                rows: [],
                fields: ["SupplierId", "name",">hutang:decimal"],
                data: {}
            },
            Items: [],
            //product_name: '',
           ...
           if (this.result!=undefined)
           {
                if (this.result.rows!=undefined)
                {
                    for (let i = 0; i < this.result.rows.length; i++) {
                        total=total + parseFloat(this.result.rows[i].hutang);
                    }                    
                }               
           }
           return total;
    ...
    async created() {
        console.log("Start mount Hutang");
    },
    methods: {
        ...
        async getDataSource(endpoint, queryParams) {
            ...
            try {
                var endpoint = this.$api.formatUrl('report/hutang', queryParams);
