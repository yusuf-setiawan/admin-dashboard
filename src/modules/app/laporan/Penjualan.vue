...
export default {
    ...
    data () {
        return {
            ...
            result:{
                rows: [],
                fields: ["id", "tanggal:date",
                {key:"name", label:"customer_name"},">qty:decimal",">total:decimal", ">total_pending:decimal", ">total_paid:decimal"],
                data: {}
            },
            Items: [],
         }
    ...
    methods: {
        ...
        async getDataSource(endpoint, queryParams) {
            ...
            try {
                var endpoint = this.$api.formatUrl('report/penjualan', queryParams);
        ...
        async onRowClick(item, index, event) {
            ...
            this.$router.push(this.$options.route+'/view/'+item.id);
        },
        ...
        async onFilter() {
            try {
                    ...
                    // var data = await this.$api.patch('report/Penjualan', model);    
                    // this.result.data = data;
                    // this.result.rows = data.rows.map(row => {
                    //     if (row.trx_type=="MutasiManual")
                    //         row.trx_code = row.trx_data.trx_code
                    //     else if (row.trx_type=="PenjualanReceive")
                    //         row.trx_code = row.trx_data.code
                    //     else   
                    //         row.trx_code = "N/A";
                    ...
                    this.title = `Laporan Penjualan Periode ${start_date} s/d ${end_date}`
               // }
            } catch (error) {
                console.log("ERROR", error);
            }   
